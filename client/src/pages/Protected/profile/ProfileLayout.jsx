...
const ProfileLayout = () => {
  ...
  const { data } = useUserDetailsQuery(params?.id, {
    skip: !params?.id, // Skip the query if no ID is provided
  });
  ...
  return (
    <>
        ...
        <title>
          {data
            ? data.user
              ...
              : "Threads Clone | App by siddharth bhatia"
            : "Threads Clone | App by siddharth bhatia"}
        </title>
      </Helmet>
      <Stack
      ...
      >
        <Link
          to={`/profile/${data?.user._id}/threads`}
          className={`link ${darkMode ? "mode" : ""}`}
        >
          Threads
        </Link>
        <Link
          to={`/profile/${data?.user._id}/replies`}
          className={`link ${darkMode ? "mode" : ""}`}
        >
          Replies
        </Link>
        <Link
          to={`/profile/${data?.user._id}/reposts`}
          className={`link ${darkMode ? "mode" : ""}`}
        >
          Reposts
        </Link>
